DECLARE
  ...
  IS
    ...
    WHERE
      lower(TRIM(CONCEPT_NAME)) = concep_name_in and
...
BEGIN
  ...
  LOOP
    ...
    nombre_fich := 'sa' || '_' || reg_summary.CONCEPT_NAME || '.load';
    ...
    UTL_FILE.put_line(fich_salida, '    FROM ''__RUTA_AL_FICHERO_CSV__'' WITH ENCODING UTF-8');
    ...
    THEN
      ...
      UTL_FILE.put_line(fich_salida, '    TARGET TABLE ' || OWNER_SA || '.sah_' || reg_summary.CONCEPT_NAME || '_' || '__FCH_DATOS__');
      ...
      UTL_FILE.put_line(fich_salida, '        work_mem to ''32 MB'', maintenance_work_mem to ''64 MB'';');
        ...
        UTL_FILE.put_line(fich_salida, '        $$ drop table if exists ' || OWNER_SA || '.sah_' || reg_summary.CONCEPT_NAME || '_' || '__FCH_DATOS__'   || '; $$,');
        UTL_FILE.put_line(fich_salida, '        $$ create table ' || OWNER_SA || '.sah_' || reg_summary.CONCEPT_NAME || '_' || '__FCH_DATOS__'   || ' (');
        UTL_FILE.put_line(fich_salida, '            like ' || OWNER_SA || '.sah_' || reg_summary.CONCEPT_NAME || ' including defaults including constraints); $$');
        UTL_FILE.put_line(fich_salida, '');
        UTL_FILE.put_line(fich_salida, '    AFTER LOAD DO');
        UTL_FILE.put_line(fich_salida, '        $$ alter table ' || OWNER_SA || '.sah_' || reg_summary.CONCEPT_NAME || ' attach partition ' || OWNER_SA || '.sah_' || reg_summary.CONCEPT_NAME || '_' || '__FCH_DATOS__');
    ...
    UTL_FILE.put_line(fich_salida_sh, '{');
    UTL_FILE.put_line(fich_salida_sh, '    insert_record_monitoreo ' || 'load_sa_' || reg_summary.CONCEPT_NAME || '.sh 1 1 0 0 0 0 0 "${FCH_DATOS}" "${INICIO_PASO_TMR}"' || ' >> "${' || NAME_DM || '_TRAZAS}"/load_sa_' || reg_summary.CONCEPT_NAME || '_"${FECHA_HORA}"' || '.log 2>' || '&' || '1');
    ...
    UTL_FILE.put_line(fich_salida_sh, '{');
    UTL_FILE.put_line(fich_salida_sh, '    insert_record_monitoreo ' || 'load_sa_' || reg_summary.CONCEPT_NAME || '.sh 1 0 "${TOT_INSERTADOS}" 0 0 "${TOT_LEIDOS}" "${TOT_RECHAZADOS}" "${FCH_DATOS}" "${INICIO_PASO_TMR}"' || ' >> "${' || NAME_DM || '_TRAZAS}"/load_sa_' || reg_summary.CONCEPT_NAME || '_"${FECHA_HORA}"' || '.log 2>&' || '1');
    UTL_FILE.put_line(fich_salida_sh, '    rc=$?');
    UTL_FILE.put_line(fich_salida_sh, '    if [ $rc -ne 0 ]');
    UTL_FILE.put_line(fich_salida_sh, '    then');
    ...
    UTL_FILE.put_line(fich_salida_sh, '  SUBJECT="Numero de paramatros de entrada incorrecto. Uso: ${0} <fch_datos> <forzado>"');
    ...
    UTL_FILE.put_line(fich_salida_sh, ')');
    UTL_FILE.put_line(fich_salida_sh, 'echo "El último paso ejecutado es: ${ULT_PASO_EJECUTADO}" >> ' || '"${' || NAME_DM || '_TRAZAS}"/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_"${FECHA_HORA}".log');
    ...
    UTL_FILE.put_line(fich_salida_sh, ')');
    UTL_FILE.put_line(fich_salida_sh, 'echo "El proceso se inicia: ${INICIO_PASO_TMR}" >> ' || '"${' || NAME_DM || '_TRAZAS}"/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_"${FECHA_HORA}".log');
    UTL_FILE.put_line(fich_salida_sh, '');
    /* (20160818) Angel Ruiz. NF: Puede existir una ruta alternativa para cargar fichero*/
    /* ya que por motivos de validacion se quiere cargar otro fichero */
    if (reg_summary.FILE_VALIDATION is null) then
    ...
    if (reg_summary.FREQUENCY = 'E') then
      UTL_FILE.put_line(fich_salida_sh, 'echo "Se trata de una carga de fichero eventual." >> ' || '"${' || NAME_DM || '_TRAZAS}"/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_"${FECHA_HORA}".log');
      ...
      UTL_FILE.put_line(fich_salida_sh, '');
      UTL_FILE.put_line(fich_salida_sh, '    InsertaFinOK');
      UTL_FILE.put_line(fich_salida_sh, '    echo "El proceso ha acabado correctamente." >> ' || '"${' || NAME_DM || '_TRAZAS}"/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_"${FECHA_HORA}".log');
      UTL_FILE.put_line(fich_salida_sh, '    exit 0');
    else
      /* Se trata de una carga eventual, por lo que a veces el fichero puede no venir y entonces no debe acabar con error */
      if (reg_summary.FILE_VALIDATION is null) then
      ...
      UTL_FILE.put_line(fich_salida_sh, '    echo date >> ' || '"${' || NAME_DM || '_TRAZAS}/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_${FECHA_HORA}".log');
      UTL_FILE.put_line(fich_salida_sh, '    InsertaFinFallido');
      UTL_FILE.put_line(fich_salida_sh, '    echo "El proceso ha acabado con error." >> ' || '"${' || NAME_DM || '_TRAZAS}"/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_"${FECHA_HORA}".log');      
      UTL_FILE.put_line(fich_salida_sh, '    exit 1');
    end if;
    UTL_FILE.put_line(fich_salida_sh, 'else');
    ...
    /* (20150605) Angel Ruiz. AÑADIDO PARA CHEQUEAR LA CALIDAD DEL DATO */
      ...
      UTL_FILE.put_line(fich_salida_sh, '  sed ''/^ *$/d'' "${' || NAME_DM || '_FUENTE}/${FCH_DATOS}/${NOMBRE_FICH_DATOS}" > "${' || NAME_DM || '_FUENTE}/${FCH_DATOS}/${NOMBRE_FICH_DATOS}".tmp');
      ...
      UTL_FILE.put_line(fich_salida_sh, '');
      UTL_FILE.put_line(fich_salida_sh, '  echo "Fichero a cargar es: ${FILE}"' || ' >> ' || '"${' || NAME_DM || '_TRAZAS}/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_${FECHA_HORA}".log');
      ...
      UTL_FILE.put_line(fich_salida_sh, '  rm "${' || NAME_DM || '_LDR}/${NOMBRE_FICH_LDR}"');
      UTL_FILE.put_line(fich_salida_sh, '');
      UTL_FILE.put_line(fich_salida_sh, '  REG_RECHAZADOS=$(grep "Total import time" ' || '"${' || NAME_DM || '_TRAZAS}/${NOMBRE_FICH_LDR_TRZ}"' || ' | awk -F'' '' '' { printf "%d",$4 } '')');
      UTL_FILE.put_line(fich_salida_sh, '  REG_RECHAZADOS=$((REG_RECHAZADOS/2)) # Se divide por 2 porque por alguna razón es el doble.');
      UTL_FILE.put_line(fich_salida_sh, '  REG_INSERTADOS=$(grep "Total import time" ' || '"${' || NAME_DM || '_TRAZAS}/${NOMBRE_FICH_LDR_TRZ}"' || ' | awk -F'' '' '' { printf "%d",$5 } '')');
      ...
      UTL_FILE.put_line(fich_salida_sh, 'fi');    
      UTL_FILE.put_line(fich_salida_sh, '');
      UTL_FILE.put_line(fich_salida_sh, 'REG_RECHAZADOS=$(grep "Total import time" ' || '${' || NAME_DM || '_TRAZAS}/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_${FECHA_HORA}.log | awk -F'' '' '' { printf "%d",$4 } '')');
      UTL_FILE.put_line(fich_salida_sh, 'REG_RECHAZADOS=$((REG_RECHAZADOS/2)) # Se divide por 2 porque por alguna razón es el doble.');
      UTL_FILE.put_line(fich_salida_sh, 'REG_INSERTADOS=$(grep "Total import time" ' || '${' || NAME_DM || '_TRAZAS}/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_${FECHA_HORA}.log | awk -F'' '' '' { printf "%d",$4 } '')');
      UTL_FILE.put_line(fich_salida_sh, 'REG_LEIDOS=$((REG_INSERTADOS+REG_RECHAZADOS))');
      UTL_FILE.put_line(fich_salida_sh, 'TOT_LEIDOS=$((TOT_LEIDOS + REG_LEIDOS))');
      UTL_FILE.put_line(fich_salida_sh, 'TOT_INSERTADOS=$((TOT_INSERTADOS + REG_INSERTADOS))');
      UTL_FILE.put_line(fich_salida_sh, 'TOT_RECHAZADOS=$((TOT_RECHAZADOS} + REG_RECHAZADOS))');
    ...
    UTL_FILE.put_line(fich_salida_sh, '# shellcheck disable=SC2129');
    UTL_FILE.put_line(fich_salida_sh, 'echo "Resumen de la carga:" >> ' || '"${' || NAME_DM || '_TRAZAS}/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_${FECHA_HORA}".log');
    UTL_FILE.put_line(fich_salida_sh, 'echo "Total registros leídos: ${TOT_LEIDOS}" >> ' || '"${' || NAME_DM || '_TRAZAS}/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_${FECHA_HORA}".log');
    UTL_FILE.put_line(fich_salida_sh, 'echo "Total registros insertados: ${TOT_INSERTADOS}" >> ' || '"${' || NAME_DM || '_TRAZAS}/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_${FECHA_HORA}".log');
    UTL_FILE.put_line(fich_salida_sh, 'echo "Total registros rechazados: ${TOT_RECHAZADOS}" >> ' || '"${' || NAME_DM || '_TRAZAS}/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_${FECHA_HORA}".log');
    ...
    if (reg_summary.FILE_VALIDATION is null) then
      /* (20160712) Angel Ruiz. CAMBIO TEMPORAL ... */
      UTL_FILE.put_line(fich_salida_sh, '# shellcheck disable=SC2129');
      UTL_FILE.put_line(fich_salida_sh, 'mv "${' || NAME_DM || '_FUENTE}/${FCH_DATOS}/' || nombre_interface_a_cargar || '" "${' || NAME_DM || '_DESTINO}/${FCH_DATOS}" >> "${' || NAME_DM || '_TRAZAS}/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_${FECHA_HORA}".log ' || '2>&' || '1');    
      UTL_FILE.put_line(fich_salida_sh, 'mv "${' || NAME_DM || '_FUENTE}/${FCH_DATOS}/' || nombre_flag_a_cargar || '" "${' || NAME_DM || '_DESTINO}/${FCH_DATOS}" >> "${' || NAME_DM || '_TRAZAS}/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_${FECHA_HORA}".log ' || '2>&' || '1');    
      /* (20160712) Angel Ruiz. FIN CAMBIO TEMPORAL*/
    else
      UTL_FILE.put_line(fich_salida_sh, '# shellcheck disable=SC2129');
      UTL_FILE.put_line(fich_salida_sh, 'mv ' || nombre_interface_a_cargar || ' "${' || NAME_DM || '_DESTINO}/${FCH_DATOS}" >> "${' || NAME_DM || '_TRAZAS}/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_${FECHA_HORA}".log ' || '2>&' || '1');    
      UTL_FILE.put_line(fich_salida_sh, 'mv ' || nombre_flag_a_cargar || ' "${' || NAME_DM || '_DESTINO}/${FCH_DATOS}" >> "${' || NAME_DM || '_TRAZAS}/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_${FECHA_HORA}".log ' || '2>&' || '1');    
    end if;
    UTL_FILE.put_line(fich_salida_sh, 'echo "La carga de la tabla ' ||  'sah_' || reg_summary.CONCEPT_NAME || ' se ha realizado correctamente." >> ' || '"${' || NAME_DM || '_TRAZAS}/' || 'load_sa' || '_' || reg_summary.CONCEPT_NAME || '_${FECHA_HORA}".log');
    UTL_FILE.put_line(fich_salida_sh, 'exit 0');    
    /******/

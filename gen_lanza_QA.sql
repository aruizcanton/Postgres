...
begin
  ...
  UTL_FILE.put_line(fich_salida_load, '#                                                                           #');
  UTL_FILE.put_line(fich_salida_load, '# Historia : 08-Oct-2025 -> Creacion                                    #');
  ...
  UTL_FILE.put_line(fich_salida_load, 'BAN_EJECUTA=$(');
  ...
  UTL_FILE.put_line(fich_salida_load, '  Y=$(grep -i VAR_DIA_CONTABILIDAD ${' || NAME_DM || '_ENTORNO}/entorno' || NAME_DM || '_' || PAIS|| '.sh | awk -F''='' ''{print $2}'')');
  UTL_FILE.put_line(fich_salida_load, '  DIA_EJECUCION=$(grep -i VAR_DIA_EJECUCION ${' || NAME_DM || '_ENTORNO}/entorno' || NAME_DM || '_' || PAIS|| '.sh | awk -F''='' ''{print $2}'')');

...
begin
  ...
  loop
    ...
    if (length(reg_quality.QUALITY_NAME) < 25) then
      ...
      --nombre_proceso := substr(reg_tabla.TABLE_NAME, 10);
      nombre_proceso := reg_quality.QUALITY_NAME;
    end if;
    ...
    UTL_FILE.put_line(fich_salida_load, '# Proposito  : Shell que ejecuta un proceso del módulo QUALITY ASSURANCE.   #');
    UTL_FILE.put_line(fich_salida_load, '# Parametros :                                                              #');
    UTL_FILE.put_line(fich_salida_load, '#                                                                           #');
    UTL_FILE.put_line(fich_salida_load, '# Ejecucion  :                                                              #');
    UTL_FILE.put_line(fich_salida_load, '#                                                                           #');
    UTL_FILE.put_line(fich_salida_load, '# Historia : 01-Octubre-2025 -> Creacion                                    #');
    UTL_FILE.put_line(fich_salida_load, '#                                                                           #');
    ...
    UTL_FILE.put_line(fich_salida_load, '{');
    UTL_FILE.put_line(fich_salida_load, '    insert_record_monitoreo ' || nombre_proceso || '.sh 1 1 0 0 0 0 0 "${FCH_DATOS}" "${FCH_CARGA}" "${INICIO_PASO_TMR}"' || ' >> "${' || NAME_DM || '_TRAZAS}"/' || nombre_proceso || '_"${FECHA_HORA}"' || '.log 2>' || '&' || '1');
    ...
    UTL_FILE.put_line(fich_salida_load, '        SUBJECT="Error en InsertarFinFallido"');
    UTL_FILE.put_line(fich_salida_load, '        echo "Error al intentar insertar un registro en el metadato."' || ' >> "${' || NAME_DM || '_TRAZAS}"/' || nombre_proceso || '_"${FECHA_HORA}"' || '.log');
    ...
    UTL_FILE.put_line(fich_salida_load, '{');
    UTL_FILE.put_line(fich_salida_load, '    insert_record_monitoreo ' || nombre_proceso || '.sh 1 0 "${TOT_INSERTADOS}" 0 0 0 0 "${FCH_DATOS}" "${FCH_CARGA}" "${INICIO_PASO_TMR}"' || ' >> "${' || NAME_DM || '_TRAZAS}"/' || nombre_proceso || '_"${FECHA_HORA}"' || '.log 2>&' || '1');
    UTL_FILE.put_line(fich_salida_load, '    rc=$?');
    UTL_FILE.put_line(fich_salida_load, '    if [ $rc -ne 0 ]');
    UTL_FILE.put_line(fich_salida_load, '    then');
    UTL_FILE.put_line(fich_salida_load, '        SUBJECT="Error en InsertarFinOK"');
    UTL_FILE.put_line(fich_salida_load, '        echo "Error al intentar insertar un registro en el metadato."' || ' >> "${' || NAME_DM || '_TRAZAS}"/' || nombre_proceso || '_"${FECHA_HORA}"' || '.log');
    --UTL_FILE.put_line(fich_salida_load, '        ${SHELL_SMS} "${TELEFONOS_DWH}" "${SUBJECT}"');
    UTL_FILE.put_line(fich_salida_load, '        exit 1;');
    UTL_FILE.put_line(fich_salida_load, '    fi');
    ...
    UTL_FILE.put_line(fich_salida_load, 'echo "${0}" > "${' || NAME_DM || '_TRAZAS}/' || nombre_proceso || '_${FECHA_HORA}"' || '.log ');
    UTL_FILE.put_line(fich_salida_load, '# shellcheck disable=SC2129');
    UTL_FILE.put_line(fich_salida_load, 'echo "Inicia Proceso: $(date +%d/%m/%Y\ %H:%M:%S)" >> "${' || NAME_DM || '_TRAZAS}/' || nombre_proceso || '_${FECHA_HORA}"' || '.log');
    UTL_FILE.put_line(fich_salida_load, 'echo "Fecha de Carga: ${FCH_CARGA}" >> "${' || NAME_DM || '_TRAZAS}/' || nombre_proceso || '_${FECHA_HORA}"' || '.log');
    UTL_FILE.put_line(fich_salida_load, 'echo "Fecha de Datos: ${FCH_DATOS}" >> "${' || NAME_DM || '_TRAZAS}/' || nombre_proceso || '_${FECHA_HORA}"' || '.log');
    UTL_FILE.put_line(fich_salida_load, 'echo "Forzado: ${BAN_FORZADO}" >> "${' || NAME_DM || '_TRAZAS}/' || nombre_proceso || '_${FECHA_HORA}"' || '.log');
    ...
    UTL_FILE.put_line(fich_salida_load, ')');
    UTL_FILE.put_line(fich_salida_load, 'FCH_CARGA_MES=${FCH_CARGA:0:6}');
    UTL_FILE.put_line(fich_salida_load, '');
    UTL_FILE.put_line(fich_salida_load, 'echo "$(date +%d/%m/%Y\ %H:%M:%S). Inicio de la carga de ' || reg_quality.QUALITY_NAME || '"' || ' >> ' || '"${' || NAME_DM || '_TRAZAS}/' || reg_quality.QUALITY_NAME || '_${FECHA_HORA}' || '.log"');
    UTL_FILE.put_line(fich_salida_load, '');
    UTL_FILE.put_line(fich_salida_load, 'sed -e "s/#VAR_FCH_REGISTRO#/${INICIO_PASO_TMR}/g" -e "s/#VAR_FCH_CARGA#/${FCH_CARGA}/g" -e "s/#VAR_FCH_DATOS#/${FCH_DATOS}/g" -e "s/#VAR_USER#/${BD_USUARIO}/g" -e "s/#VAR_CVE_MES#/${FCH_CARGA_MES}/g" -e "s/#VAR_CVE_DIA#/${FCH_CARGA}/g" "${ANA_SQL}/' || reg_quality.QUALITY_NAME || '.sql" > "${ANA_TMP}/' || reg_quality.QUALITY_NAME || '_tmp.sql"');
    UTL_FILE.put_line(fich_salida_load, 'echo "$(date +%d/%m/%Y\ %H:%M:%S). La SQL ejecutadada es :" >> "${' || NAME_DM || '_TRAZAS}/' || reg_quality.QUALITY_NAME || '_${FECHA_HORA}' || '.log"');
    UTL_FILE.put_line(fich_salida_load, 'cat "${ANA_TMP}/' || reg_quality.QUALITY_NAME || '_tmp.sql" >> "${' || NAME_DM || '_TRAZAS}/' || reg_quality.QUALITY_NAME || '_${FECHA_HORA}' || '.log"');
    ...
    if (reg_quality.FILE_SPOOL = 'S') then
      /* (20170509) Angel Ruiz. Se trata de un proceso de calidad que genera un fichero de contraste */
      UTL_FILE.put_line(fich_salida_load, 'psql -h "$HOST" -p "$PORT" -U "$BD_USUARIO" -d "$DB_NAME" -t -A -v ON_ERROR_STOP=1 -F"|" -f "' || '"${' || NAME_DM || '_TMP}/' || reg_quality.QUALITY_NAME || '_tmp.sql" > "${' || NAME_DM || '_TMP}/' || reg_quality.QUALITY_NAME || '_${FCH_DATOS}.dat" 2>> "${' || NAME_DM || '_TRAZAS}/' || reg_quality.QUALITY_NAME || '_${FECHA_HORA}.log");');
    else
      /* (20170509) Angel Ruiz. Se trata de un proceso de calidad que no genera un fichero de contraste */
      UTL_FILE.put_line(fich_salida_load, 'TOT_INSERTADOS=$(psql -h "$HOST" -p "$PORT" -U "$BD_USUARIO" -d "$DB_NAME" -t -A -v ON_ERROR_STOP=1 -f "' || '${' || NAME_DM || '_TMP}/' || reg_quality.QUALITY_NAME || '_tmp.sql" 2>> "${' || NAME_DM || '_TRAZAS}/' || reg_quality.QUALITY_NAME || '_${FECHA_HORA}.log")');
    end if;
    UTL_FILE.put_line(fich_salida_load, 'ERROR=$?');
    UTL_FILE.put_line(fich_salida_load, '');
    UTL_FILE.put_line(fich_salida_load, 'if [ ${ERROR} != 0 ] ; then');
    UTL_FILE.put_line(fich_salida_load, '  SUBJECT="Surgio un error en el proceso de calidad ' || reg_quality.QUALITY_NAME || '. Error: ${ERROR}."');
    --UTL_FILE.put_line(fich_salida_load, '  ${SHELL_SMS} "${TELEFONOS_DWH}" "${SUBJECT}"');
    UTL_FILE.put_line(fich_salida_load, '  echo "${SUBJECT}" >> ' || '"${' || NAME_DM || '_TRAZAS}/' || reg_quality.QUALITY_NAME || '_${FECHA_HORA}.log"');
    UTL_FILE.put_line(fich_salida_load, '  date >> ' || '"${' || NAME_DM || '_TRAZAS}/' || reg_quality.QUALITY_NAME || '_${FECHA_HORA}.log"');
    UTL_FILE.put_line(fich_salida_load, '  InsertaFinFallido');
    UTL_FILE.put_line(fich_salida_load, '  exit 1');    
    UTL_FILE.put_line(fich_salida_load, 'fi');
    UTL_FILE.put_line(fich_salida_load, '');
    UTL_FILE.put_line(fich_salida_load, '# Borro el fichero temporal .sql generado en vuelo');
    UTL_FILE.put_line(fich_salida_load, 'rm "${' || NAME_DM || '_TMP}/' || reg_quality.QUALITY_NAME || '_tmp.sql"');
    if (reg_quality.FILE_SPOOL = 'S') then
      /* (20170509) Angel Ruiz. Se trata de un proceso de calidad que genera un fichero de contraste */
      UTL_FILE.put_line(fich_salida_load, '# Surpimimos las lineas en blanco del fichero obtenido');
      UTL_FILE.put_line(fich_salida_load, 'grep -G -v -e ''^.$'' -e ''^[ ]*.$'' -e ''^$'' ' || '"${' || NAME_DM || '_TMP}/' || reg_quality.QUALITY_NAME || '_${FCH_DATOS}.dat" > "${' || NAME_DM || '_TMP}/' || reg_quality.QUALITY_NAME || '_${FCH_DATOS}.dat_tmp"');
      UTL_FILE.put_line(fich_salida_load, 'rm -f "${' || NAME_DM || '_TMP}/' || reg_quality.QUALITY_NAME || '_${FCH_DATOS}.dat"');
      UTL_FILE.put_line(fich_salida_load, 'mv "${' || NAME_DM || '_TMP}/' || reg_quality.QUALITY_NAME || '_${FCH_DATOS}.dat_tmp" "${' || NAME_DM || '_TMP}/' || reg_quality.QUALITY_NAME || '_${FCH_DATOS}.dat"');
      UTL_FILE.put_line(fich_salida_load, '# Procesamos el fichero obtenido');
      UTL_FILE.put_line(fich_salida_load, '# Comprobamos si existe el directorio de CALIDAD para fecha de carga');
      UTL_FILE.put_line(fich_salida_load, 'if [ ! -d "${' || NAME_DM || '_PATH}/${ANA_PAIS}/Calidad" ] ; then');
      UTL_FILE.put_line(fich_salida_load, '  mkdir "${' || NAME_DM || '_PATH}/${ANA_PAIS}/Calidad"');
      UTL_FILE.put_line(fich_salida_load, 'fi');
      UTL_FILE.put_line(fich_salida_load, 'if [ ! -d "${' || NAME_DM || '_PATH}/${ANA_PAIS}/Calidad/${FCH_CARGA}" ] ; then');
      UTL_FILE.put_line(fich_salida_load, '  mkdir "${' || NAME_DM || '_PATH}/${ANA_PAIS}/Calidad/${FCH_CARGA}"');
      UTL_FILE.put_line(fich_salida_load, 'fi');
      UTL_FILE.put_line(fich_salida_load, '# Generamos el fichero extraido en el destino');
      UTL_FILE.put_line(fich_salida_load, 'NUM_FILES=$(ls -1 ${' || NAME_DM || '_TMP}/' || reg_quality.QUALITY_NAME || '_${FCH_DATOS}.dat | wc -l)');
      UTL_FILE.put_line(fich_salida_load, 'if [ ${NUM_FILES} -eq 1 ]; then');
      UTL_FILE.put_line(fich_salida_load, '  mv ${' || NAME_DM || '_TMP}/' || reg_quality.QUALITY_NAME || '_${FCH_DATOS}.dat ${' || NAME_DM || '_PATH}/${ANA_PAIS}/Calidad/${FCH_CARGA}/' || reg_quality.QUALITY_NAME || '_${FCH_DATOS}.dat');
      UTL_FILE.put_line(fich_salida_load, 'fi');
    end if;
    UTL_FILE.put_line(fich_salida_load, '');
    ...
    if (reg_quality.FILE_SPOOL = 'S') then
      ...
      UTL_FILE.put_line(fich_salida_load, 'TOT_INSERTADOS=$(wc -l < ${' || NAME_DM || '_PATH}/${ANA_PAIS}/Calidad/${FCH_CARGA}/' || reg_quality.QUALITY_NAME || '_${FCH_DATOS}.dat)');
    end if;
    UTL_FILE.put_line(fich_salida_load, 'echo "$(date +%d/%m/%Y\ %H:%M:%S). Total de registros insertados: ${TOT_INSERTADOS}" >> ' || '"${' || NAME_DM || '_TRAZAS}/' || reg_quality.QUALITY_NAME || '_${FECHA_HORA}.log"');
    UTL_FILE.put_line(fich_salida_load, 'echo "$(date +%d/%m/%Y\ %H:%M:%S). Después de la ejecución de la query de calidad de manera exitosa." >> ' || '"${' || NAME_DM || '_TRAZAS}/' || reg_quality.QUALITY_NAME || '_${FECHA_HORA}.log"');
    UTL_FILE.put_line(fich_salida_load, '# Insertamos que el proceso se ha Ejecutado Correctamente');
    UTL_FILE.put_line(fich_salida_load, 'InsertaFinOK');
    UTL_FILE.put_line(fich_salida_load, '');
    UTL_FILE.put_line(fich_salida_load, 'echo "$(date +%d/%m/%Y\ %H:%M:%S). El proceso ' || reg_quality.QUALITY_NAME || ' se ha realizado correctamente." >> ' || '"${' || NAME_DM || '_TRAZAS}/' || reg_quality.QUALITY_NAME || '_${FECHA_HORA}.log"');
    UTL_FILE.put_line(fich_salida_load, '');
